...
def users_list(request):
 ...
 """
    if request.method == 'GET':
        ...
        serializer = UserAppSerializer(data,context={'request': request} ,many=True)
        if data.has_next():
...
def users_detail(request, pk):
 ...
 """
    ...
    if request.method == 'GET':
        serializer = UserAppSerializer(user,context={'request': request})
        return Response(serializer.data)

    elif request.method == 'PUT':
        serializer = UserAppSerializer(user, data=request.data,context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
